...
def estimate_interventional_probability_tabular(model, X_train, x_instance, intervention_features):
    tmp = X_train.copy()
    intervention_values = x_instance[intervention_features]    
...
class PrototypeExplainer(ABC):
    def __init__(self, *, model, actions, causal_model:CausalModel, utility_matrix, X_train, cognitive_method, information_method, base=np.e, random_seed=42, **kwargs):
        assert actions == causal_model.actions, "Invalid causal model"
        self.model = model
...
class PrototypeTabularExplainer(PrototypeExplainer):
    def __init__(self, features, model, actions, causal_model:CausalModel, utility_matrix, X_train, minimalism = False, cognitive_method:Sequence[Literal["quantity", "time"]]=("quantity",), information_method:Literal["shannon", "gini"]="shannon", base=np.e, random_seed=42, ohe_group = dict(), **kwargs):
        super().__init__(model, actions, causal_model, utility_matrix, X_train, cognitive_method, base, random_seed=random_seed)
    ...
    def explain_instance(self, x_instance, max_k_features = 5):
        ...
        self.explanations.sort(reverse=True, key=lambda e:(e["utility score"], e["information score"], -len(e["features"]) if self.minimalism else len(e["features"])))
